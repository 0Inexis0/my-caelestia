...
# variables.lua, scheme/default.lua, scripts/), preserving the relative path.
# monitors.d is handled separately below since those are machine-specific.
# scheme/current.lua is NOT tracked (generated at runtime by the theme tool;
# hyprland.lua seeds it from default.lua on first launch).
